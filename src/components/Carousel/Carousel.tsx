...
const DEFAULT_CONFIG: CarouselConfig = { interactiveItems: 1 };
const Carousel = ({
  className = "",
  data,
  title,
  ...
  useEffect(() => {
    const observer = new ResizeObserver(([entry]) => {
      setCarouselComputedValues((prev) => {
        ...
        const carouselStripWidth =
          refCarouselContent.current?.getBoundingClientRect().width || 0;
        ...
        console.log("tttt", refCarouselContent.current);
        console.log(
          "..---->",
          carouselWidth,
          interactiveItems,
          carouselWidth / interactiveItems,
          carouselItemWidth,
          carouselItemWidthWithGap,
          computedGap,
          `carouselStripWidth: ${carouselStripWidth}`
        );
        ...
        if (data.length <= interactiveItems)
          ...
          for (; i < numberOfSections; i++) {
            ...
            const itemsCount = (i + 1) * interactiveItems;
